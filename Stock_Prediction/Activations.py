...
class Network(object):
    ...
    def train(self, training_set, desired_outputs):
        ...
        while True:
            ...
            for pattern in training_set:
                ...
                self.feed_forward()
                self.calc_output_errors(desired_outputs[training_set.index(pattern)])
                self.calculate_global_error()
                self.back_prop()
            self.update_weights()
            if (abs(self.global_error - preverr)) <= 0.001 or epoch == 100:
                break
            else:
                preverr = self.global_error
                self.global_error = 0.0
    ...
    def test(self, pattern):
        self.impose_pattern(pattern)
        self.feed_forward()
        return self.layers[-1].nodes[0].activation
        # self.global_error = 0.0
